...
client.ws.on('INTERACTION_CREATE', async interaction => {
    ...
    try {
        async function send(content, type, flags) {
            client.api
        ...
        async function sendEmbed(embed, type, flags) {
            client.api
                .interactions(id)(token)
                .callback
                .post({data: {
        ...
        async function edit(content) {
            client.api
                .webhooks((await client.fetchApplication()).id)(token)
                .messages('@original')
                .patch({data: {
                    content: content
                }})
        ...
        switch (data.name) {
            ...
            case 'avatar': {
                const user_id = data.options ? data.options[0].value : user.id;
                const user_av = client.users.cache.get(user_id);
            ...
            case 'ban': {
                ...
                if (author.hasPermission('BAN_MEMBERS') === false) {
                    send(`<:red_x:717257458657263679> Error: \`DiscordAPIError: Missing Permissions\``, 3, 64);
                    return;
                }
                ...
                await member.ban({days: days, reason: reason})
                    .catch((err) => {
                        error = true;
                        logger.error(err);
                        send(`<:red_x:717257458657263679> Error: \`${err}\``, 3, 64);
                    });

                if (error === false) {
                    switch (silent) { 
                        case false:
                            send(`${displayMember} has been banned from the server for reason: \`${displayReason}\``, 4, 0);
                            break;
                            
                        case true:
                            send(`${displayMember} has been banned from the server for reason: \`${displayReason}\``, 3, 64);
                            break;
                    }
                }
            ...
            case 'kick': {
                ...
                if (error === false) {
                    switch (silent) { 
                        case false:
                            send(`${displayMember} has been kicked from the server for reason: \`${displayReason}\``, 4, 0);
                            break;
                            
                        case true:
                            send(`${displayMember} has been kicked from the server for reason: \`${displayReason}\``, 3, 64);
                            break;
                    }
                }
            ...
            case 'remind': {
                ...
                switch (type) {
                    ...
                    case 'duration':
                        ...
                        for (let i = 0; i < duration.length; i++) {
                            switch (duration[i]) {
                                case 'year':
                                    years = duration[i - 1] * 365 * 24 * 60 * 60 * 1000;
                                    break;
                                ...
                                case 'month':
                                    months = duration[i - 1] * 30 * 24 * 60 * 60 * 1000;
                                    break;
                                ...
                                default:
                                    if (duration[i].endsWith('y')) years = duration[i].substring(0, duration[i].length - 1) * 365 * 24 * 60 * 60 * 1000;
                                    else if (duration[i].endsWith('mo')) months = duration[i].substring(0, duration[i].length - 2) * 30 * 24 * 60 * 60 * 1000;
                                    else if (duration[i].endsWith('w')) weeks = duration[i].substring(0, duration[i].length - 1) * 7 * 24 * 60 * 60 * 1000;
                                    else if (duration[i].endsWith('d')) days = duration[i].substring(0, duration[i].length - 1) * 24 * 60 * 60 * 1000;
                                    else if (duration[i].endsWith('h')) hours = duration[i].substring(0, duration[i].length - 1) * 60 * 60 * 1000;
                                    else if (duration[i].endsWith('m')) minutes = duration[i].substring(0, duration[i].length - 1) * 60 * 1000;
                                    else if (duration[i].endsWith('s')) seconds = duration[i].substring(0, duration[i].length - 1) * 1000;
                            }
                        }
                ...
                if (isNaN(timeout)) {
                    send(`<:red_x:717257458657263679> Error: \`InputError: Invalid Time\``, 3, 64);

                } else {
                    send(`Okay, I'll remind ${displayPlace} about: \`${reminder}\` ${displayTime}`, 4, 0);
                    setTimeout(() => {
